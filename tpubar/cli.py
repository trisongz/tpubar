...
def test_tpubar(tpu_name):
    ...
    if env['colab']:
        monitor = TPUMonitor(tpu_name=tpu_name, profiler='v2', refresh_secs=3, verbose=True)
    else:
        monitor = TPUMonitor(tpu_name=tpu_name, profiler='v1', refresh_secs=3, verbose=True)
